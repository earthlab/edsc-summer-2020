...
# <div class="alert alert-info"><h4>Note on Memory Allocation</h4><p>To understand the difference between <code>is</code> and <code>==</code>, first you need to know a little bit about how Python stores data. In Python, when a variable is declared, there's a certain place on your computer that is reserved to store that data. This place has a memory address. In Python, you can see the memory id of a variable using the <code>id()</code> function. This will print out a long integer that is how Python identifies the object. It's what Python uses to find the variable in the computer's memory. 
#
# When you create a variable, let's say <code>a = 3</code>, if you run <code>id(a)</code> or <code>id(3)</code>, you'll get the same output. For me it was <code>94641000121024</code>. This is the memory address where Python stores the variable <code>3</code> for your script. 
#     
# When you create a list, a new <code>id</code> is created for your list, as it is a new variable in Python. So let's say you mak a list like so: <code>list1 = [1, 2, 3]</code>. This list would be given a new id, such as <code>140657632719088</code>. If you create an identical list like this: <code>list2 = [1, 2, 3]</code> and check its <code>id</code>, it's stored at a different location, such as <code>140657632694912</code>. This is because the list is a new variable that's stored in a new place in the memory. Because of this, <code>list1 == list2</code> will return <code>True</code>, because their values are equal. However, <code>list1 is list2</code> will return <code>False</code>, since they are stored in different parts of your computers memory.
#     
# However, if you make a list by assigning it to the old list, a.k.a <code>list1 = list2</code>, they will both have the memory address of <code>list1</code>, <code>140657632719088</code>. This is because you were not creating a new variable, and thus making a new memory address. You were instead pointing to an old memory address when creating <code>list2</code>. Because of this, <code>list1 == list2</code> will return <code>True</code>, because their values are equal, AND <code>list1 is list2</code> will also return <code>True</code>, since they are stored at the same memory address.</p></div>
