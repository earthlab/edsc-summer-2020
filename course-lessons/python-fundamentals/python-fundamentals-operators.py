...
#
# When you create a variable, let's say `a = 3`, if you run `id(a)` or `id(3)`, you'll get the same output. For me it was `94641000121024`. This is the memory address where Python stores the variable `3` for your script. 
#     
# When you create a list, a new `id` is created for your list, as it is a new variable in Python. So let's say you mak a list like so: `list1 = [1, 2, 3]`. This list would be given a new id, such as `140657632719088`. If you create an identical list like this: `list2 = [1, 2, 3]` and check its `id`, it's stored at a different location, such as `140657632694912`. This is because the list is a new variable that's stored in a new place in the memory. Because of this, `list1 == list2` will return `True`, because their values are equal. However, `list1 is list2` will return `False`, since they are stored in different parts of your computers memory.
#     
# However, if you make a list by assigning it to the old list, a.k.a `list1 = list2`, they will both have the memory address of `list1`, `140657632719088`. This is because you were not creating a new variable, and thus making a new memory address. You were instead pointing to an old memory address when creating `list2`. Because of this, `list1 == list2` will return `True`, because their values are equal, AND `list1 is list2` will also return `True`, since they are stored at the s.</div>
